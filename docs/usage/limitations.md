...
- **Graph mode has routine latency.** The bridge triggers an immediate run right after it uploads a transcript; the routine's cron (`GRAPH_ROUTINE_CRON`, default hourly) is the safety net for when that immediate trigger fails, so a transcript can occasionally sit in `uploads/` up to an hour, not immediate.
